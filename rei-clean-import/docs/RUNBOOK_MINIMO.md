...
## 0. Build y Despliegue (Next.js Static)

El proyecto utiliza **Next.js Static Export**.

- **Comando de Build Real**: `npm run build`
- **Output Folder Real**: `out/` (Static Export standard)
- **Destino en Servidor**: `/var/www/rei/current` debe apuntar a este contenido.
- **NOTA IMPORTANTE**: El contenido debe tener permisos `www-data:www-data` (755 dirs, 644 files) para que Caddy pueda servirlo.

## 0.1. Despliegue Automatizado

Se ha creado un script para automatizar build, upload y rotación de releases.
...
2. Subir a nueva carpeta `/var/www/rei/releases/<TIMESTAMP>`.
3. Actualizar symlink `current` y recargar Caddy.
